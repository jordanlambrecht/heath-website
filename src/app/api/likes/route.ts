...
async function getPool() {
  ...
  // @ts-ignore - 'pg' may not have types installed in this environment; handle missing module at runtime
...
export async function POST(req: Request) {
  ...
  try {
    ...
    ;(global as any).__likes_rate_limits = (global as any).__likes_rate_limits || new Map()
    ...
    if (pool) {
      ...
      const result = await pool.query(sql, [delta, isNumericId ? Number(poemId) : String(poemId)])
    ...
    try {
      ...
      const currentLikes = (existing as any)?.likes || 0
      const newLikes = act === 'like' ? currentLikes + 1 : Math.max(0, currentLikes - 1)
      // cast to any to allow updating newly-added 'likes' field without generated types
